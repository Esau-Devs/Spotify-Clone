...
export const VolumeSilenced = ({ className }) => (
    <svg className={className} fill="currentColor" role="presentation" height="16" width="16" aria-hidden="true" aria-label="Volumen apagado"
        viewBox="0 0 16 16">
        <path
...
export const VolumeLow = ({ className }) => (
    <svg className={className} fill="currentColor" role="presentation" height="16" width="16" aria-label="Volumen bajo" aria-hidden="true"
        id="volume-icon"
        viewBox="0 0 16 16">
        <path
...
export const VolumeMedium = ({ className }) => (
    <svg className={className} fill="currentColor" role="presentation" height="16" width="16" aria-label="Volumen medio" aria-hidden="true"
        id="volume-icon"
        viewBox="0 0 16 16">
        <path
...
const SongControl = ({ audio }) => {
    ...
    const formatTime = (time) => {
        ...
        const segunds = Math.floor(time % 60);
        const minutos = Math.floor(time / 60);

        return `${minutos}:${segunds.toString().padStart(2, '0')}`;
    }
...
const VolumenControl = () => {
    ...
    const handleClickVolumen = () => {
        ...
        if (isVolumenSilence) {
            ...
            setVolume(previousVolumenRef.current = volume)
            setVolume(0)
        }
    ...
    return (
        <div className='flex justify-center gap-x-2 text-white'>
            ...
            <button className='cursor-pointer  opacity-70 hover:opacity-100 transition' onClick={handleClickVolumen} >
                {isVolumenSilence ? <VolumeSilenced /> : <VolumeFull />}
